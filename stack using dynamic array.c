#include<stdlib.h>    //stack using dynamic array
#include<conio.h>
#include<stdio.h>
struct stack
{
...
void pop(struct stack *sta)
{
  ...
  if(sta->lastindex+1<=sta->capacity/2&&sta->capacity>1)
  halfstack(sta);

}
doublestack(struct stack *sta)
{
    sta->capacity*=2;
    sta->ptr=realloc(sta->ptr,sta->capacity*sizeof(struct stack));
}
halfstack(struct stack *sta)
{
    sta->capacity/=2;
    sta->ptr=realloc(sta->ptr,sta->capacity*sizeof(struct stack));
}
