#include<stdlib.h>    //using dynamic array
#include<conio.h>
#include<stdio.h>
...
deletion(struct queue *q)
{
    ...
    if(queuecount(q)<=q->capacity/2&&q->capacity>1)
        halfqueue(q);
}
view(struct queue *q)
{
...
else
{
    ...
    printf("queuecount = %d\n",queuecount(q));
    printf("front value= %d , rear value= %d , capacity = %d\n\n",q->ptr[q->front],q->ptr[q->rear],q->capacity);
}
}
...
doublequeue(struct queue *q)
{
    int i,j;
    int *temp=malloc(sizeof(int)*(q->capacity)*2);
